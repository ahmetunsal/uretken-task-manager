...
function TaskForm() {
    ...
    async function handleInputChange(e) {
        ...
        setTaskLocalUpdateCount(prev=>prev+1);
        if(form.task.length < 3) setIsFilledBlanks(false)
        else setIsFilledBlanks(true)
        if(form.description.length < 3) setisFilledBlanksForDesc(false)
        else setisFilledBlanksForDesc(true)
        console.log(form)
    }
    ...
    function handleFormSubmit(event) {
        ...
        if (form.isEdited) {
            if (tasks && form.alt === "task") {
                const taskIndex = tasks.findIndex(x => x.uuid === form.uuid);
                const newTask = tasks.slice()
                newTask[taskIndex] = { ...form }
                setTasks(newTask)
            } else {
                const taskIndex = complatedTasks.findIndex(x => x.uuid === form.uuid);
                const newTask = complatedTasks.slice()
                newTask[taskIndex] = { ...form }
                setComplatedTasks(newTask)
            }
